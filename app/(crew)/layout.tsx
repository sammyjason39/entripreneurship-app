...
export default async function CrewLayout({ children }: { children: React.ReactNode }) {
  const { user, profile } = await requireCrewOrAdmin();
  const isAdmin = profile.app_role === 'admin';
  const assignment = isAdmin ? null : await getCrewAssignment(user.id);
  const permissions = getCrewPermissions(profile, assignment);

  return (
    <div className={isAdmin ? 'mx-auto min-h-dvh w-full max-w-6xl' : 'mx-auto min-h-dvh w-full max-w-[480px]'}>
      <header className="sticky top-0 z-40 flex items-center justify-between border-b-2 border-border bg-bg-primary px-4 py-3 text-on-bg-readable">
        <div>
          ...
          {isAdmin ? (
            <Link href="/admin" className="font-body text-[10px] text-accent-green">
              ← Admin dashboard
            </Link>
          ) : (
      ...
      <div className={isAdmin ? 'pb-10 pt-2' : 'pb-24 pt-2'}>{children}</div>
      {!isAdmin && <CrewBottomNav permissions={permissions} />}
    </div>
  );
}
